  ...
  for (const row of rows) {
    ...
    for (const [date, mark] of Object.entries(row.marks)) {
      ...
      if (letters && X_RE.test(letters)) {
        ambiguous.push({ date, boks: row.boks, mark, reason: "samo X w boksie z wieloma psami" });
        continue;
      }
      ...
      for (const t of tokens) {
        ...
        // "KM" = K + M — każda litera musi wskazywać dokładnie jednego psa
        const perLetter = [];
        let ok = tn.length > 0;
        for (const ch of tn) {
      ...
      if (anyAmb) {
        ambiguous.push({ date, boks: row.boks, mark, reason: "oznaczenie bez jednoznacznego dopasowania" });
      }
    }
  }
