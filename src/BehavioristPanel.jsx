...
const slotLabel = (s) => `${TYPE_LABEL[s.type] || s.type} ${s.slot}`;
      ...
      {dashboard.slots.map((slot) => (
        <div key={`${slot.type}-${slot.slot}`} style={styles.card}>
          <div style={styles.row}>
            <div>
              <div style={{ fontWeight: 800 }}>{slotLabel(slot)}</div>
              <div style={{ fontSize: 13, color: "#4b5563" }}>{slot.dog ? `${slot.dog.name} | ${slot.dog.id} | boks ${slot.dog.kennel}` : "Brak przypisania"}</div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>Status: {STATUS_LABEL[slot.status] || slot.status || "Puste"}</div>
            </div>
            <button style={{ ...styles.btn, background: "#dbeafe", color: "#1d4ed8" }} onClick={() => onOpenSession(slot)} disabled={!slot.dogId}>Rozpocznij sesję</button>
          </div>
        </div>
      ))}
    </div>
    ...
    <div style={styles.page}>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.tabs}>
          {[ ["today", "Dziś"], ["week", "Plan tygodnia"], ["inbox", "Skrzynka"], ["todo", "Do zaplanowania"], ["dog", "Karta psa"] ].map(([key, text]) => (
        ...
        {tab === "today" && (
          <TodayView
            dashboard={dashboard}
            onOpenSession={(slot) => runAction(async () => {
              const therapyDog = await apiGet(currentUser, "getTherapyDog", { dogId: slot.dogId });
              const exercises = String(therapyDog?.Exercises || "").split("\n").map((line) => line.trim()).filter(Boolean);
              setSessionModal({ slot, exercises, exercisesDone: [], note: "" });
            })}
          />
        )}
        ...
        {tab === "week" && (
          <div>
            <div style={styles.card}><div style={styles.row}><input type="date" value={date} onChange={(e) => setDate(e.target.value)} style={styles.input} /><button style={{ ...styles.btn, background: "#2563eb", color: "white" }} onClick={() => runAction(async () => { await apiPost(currentUser, "autoPlanWeek", { startDate: date, days: 7 }); await loadAll(); }, "Plan tygodnia został ułożony.")}>Ułóż tydzień</button></div></div>
            {week.map((day) => (
              <div key={day.date} style={styles.card}>
                <div style={{ fontWeight: 800, marginBottom: 8 }}>{day.date}</div>
                {SLOT_TYPES.map((slotDef) => {
                  const slot = day.byKey[`${slotDef.type}-${slotDef.slot}`] || {};
                  const dog = dogsById[String(slot.dogId)];
                  return (
                    <div key={`${day.date}-${slotDef.type}-${slotDef.slot}`} style={{ display: "grid", gridTemplateColumns: "160px 1fr auto auto", gap: 8, marginBottom: 6, alignItems: "center" }}>
                      <div style={{ fontWeight: 600 }}>{slotLabel(slotDef)}</div>
                      <div style={{ fontSize: 13 }}>{dog ? `${dog.name} (${dog.id}, boks ${dog.kennel})` : "puste"}</div>
                      <input placeholder="ID psa" defaultValue={slot.dogId || ""} style={styles.input} onBlur={(e) => runAction(async () => { const dogId = e.target.value.trim(); if (!dogId) return; await apiPost(currentUser, "setScheduleSlot", { date: day.date, type: slotDef.type, slot: slotDef.slot, dogId }); await loadAll(); }, "Zaktualizowano plan.")} />
                      <button style={{ ...styles.btn, background: "#f3f4f6" }} onClick={() => runAction(async () => { await apiPost(currentUser, "clearScheduleSlot", { date: day.date, type: slotDef.type, slot: slotDef.slot }); await loadAll(); }, "Wyczyszczono slot.")}>Wyczyść</button>
                    </div>
                  );
                })}
              </div>
        ...
        {tab === "inbox" && inbox.map((item) => (
          <div key={item.id} style={styles.card}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>{[["THERAPY", "Terapia"], ["WAIT", "Oczekuje"], ["REJECTED", "Odrzucony"]].map(([value, text]) => <button key={value} style={{ ...styles.btn, background: "#e5e7eb" }} onClick={() => runAction(async () => { await apiPost(currentUser, "setTherapyDecision", { dogId: item.dogId, decision: value }); await loadAll(); }, "Zmieniono decyzję.")}>{text}</button>)}</div>
          </div>
        ))}
        ...
        {tab === "dog" && (
          <div>
            ...
            {filteredDogs.slice(0, 30).map((dog) => <div key={dog.id} style={styles.card} onClick={() => runAction(async () => { const dogId = String(dog.id); setSelectedDogId(dogId); setDogCard(await apiGet(currentUser, "getTherapyDog", { dogId })); })}>{dog.name} ({dog.id}) boks {dog.kennel}</div>)}
            {selectedDogId && dogCard ? (
              <div style={styles.card}>
                <div style={{ fontWeight: 800, marginBottom: 8 }}>Karta terapii: {selectedDogId}</div>
                <select value={dogCard.TherapyStatus || "WAIT"} style={styles.input} onChange={(e) => setDogCard((prev) => ({ ...prev, TherapyStatus: e.target.value }))}>{THERAPY_STATUS_OPTIONS.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select>
                ...
                <button style={{ ...styles.btn, background: "#2563eb", color: "white" }} onClick={() => runAction(async () => { await apiPost(currentUser, "updateTherapyDog", { dogId: selectedDogId, therapyStatus: dogCard.TherapyStatus, priority: Number(dogCard.Priority), walkFreqDays: dogCard.WalkFreqDays, boxFreqDays: dogCard.BoxFreqDays, workAreas: dogCard.WorkAreas, exercises: dogCard.Exercises, notesShort: dogCard.NotesShort || "" }); await loadAll(); }, "Zapisano kartę psa.")}>Zapisz kartę</button>
              </div>
            ) : null}
          </div>
        )}
        ...
        {sessionModal ? (
          <div style={{ position: "fixed", inset: 0, background: "rgba(17,24,39,0.55)", padding: "1rem", zIndex: 100 }}>
            <div style={{ ...styles.card, margin: "4rem auto", maxWidth: 540 }}>
              <div style={{ fontWeight: 800, marginBottom: 8 }}>Sesja: {slotLabel(sessionModal.slot)}</div>
              <div style={{ marginBottom: 8, color: "#4b5563" }}>{dogsById[String(sessionModal.slot.dogId)]?.name || "Nieznany pies"}</div>
              {sessionModal.exercises.length ? sessionModal.exercises.map((exercise) => (
                <label key={exercise} style={{ display: "flex", gap: 8, marginBottom: 6 }}><input type="checkbox" onChange={(e) => setSessionModal((prev) => ({ ...prev, exercisesDone: e.target.checked ? [...prev.exercisesDone, exercise] : prev.exercisesDone.filter((x) => x !== exercise) }))} /> {exercise}</label>
              )) : <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 8 }}>Brak zdefiniowanych ćwiczeń na karcie psa.</div>}
              <textarea style={{ ...styles.input, minHeight: 100 }} placeholder="Notatka" onChange={(e) => setSessionModal((p) => ({ ...p, note: e.target.value }))} />
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button style={{ ...styles.btn, background: "#16a34a", color: "white" }} onClick={() => runAction(async () => { await apiPost(currentUser, "completeSession", { date: sessionModal.slot.date, type: sessionModal.slot.type, slot: sessionModal.slot.slot, dogId: sessionModal.slot.dogId, exercisesDone: sessionModal.exercisesDone, note: sessionModal.note }); setSessionModal(null); await loadAll(); }, "Sesja została zapisana.")}>Zakończ sesję</button>
