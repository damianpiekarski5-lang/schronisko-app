...
function BehavioristPanel({ currentUser, dogs, onBack }) {
  ...
  const [therapyDogs, setTherapyDogs] = useState([]);
  ...
  const loadAll = async () => {
    try {
      const [dayData, weekData, inboxData, scheduleData, therapyDogsData] = await Promise.all([
        ...
        apiGet(currentUser, "getTherapyDogs"),
      ]);
      ...
      setTherapyDogs(therapyDogsData || []);
    } catch (e) {
  ...
  const dogSource = dogs.length ? dogs : therapyDogs.map((row) => ({ id: row.DogID, kennel: row.box, name: row.name }));

  const filteredDogs = dogSource.filter((dog) => {
          ...
          <div>
            ...
            <div style={styles.card}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8 }}>
                <input value={manualDogId} onChange={(e) => setManualDogId(e.target.value)} style={styles.input} placeholder="Dodaj kartę po DogID" />
                <button style={{ ...styles.btn, background: "#2563eb", color: "white" }} onClick={async () => {
                  const dogId = manualDogId.trim();
                  if (!dogId) return;
                  await apiPost(currentUser, "updateTherapyDog", { dogId });
                  setSelectedDogId(dogId);
                  setDogCard(await apiGet(currentUser, "getTherapyDog", { dogId }));
                  setManualDogId("");
                  loadAll();
                }}>Dodaj kartę</button>
              </div>
            </div>
            {filteredDogs.slice(0, 50).map((dog) => <div key={dog.id} style={styles.card} onClick={async () => { setSelectedDogId(dog.id); setDogCard(await apiGet(currentUser, "getTherapyDog", { dogId: dog.id })); }}>{dog.name || "(bez imienia)"} ({dog.id}) boks {dog.kennel || "-"}</div>)}
