...
function doGet(e) {
  ...
  try {
    ...
    if (action === "getTherapyDogs") {
      return json({ ok: true, data: getTherapyDogs_() });
    }
...
function findHeaderIndex_(headers, aliases) {
  const normalized = headers.map((h) => safeStr(h).toLowerCase());
...
function getTherapyInbox_() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const reportsSheet = getOrCreateSheet(ss, REPORTS_SHEET_NAME);
  ensureHeadersAppend_(reportsSheet, ["therapyDecision", "therapyDecisionAt"]);

  const values = reportsSheet.getDataRange().getValues();
  if (values.length <= 1) return [];

  const headers = values[0].map((h) => safeStr(h));
  const idxDogId = findHeaderIndex_(headers, ["dogId", "DogID", "ID psa", "id_psa"]);
  const idxDogName = findHeaderIndex_(headers, ["dogName", "dog", "Imię psa", "imie psa", "name"]);
  const idxReason = findHeaderIndex_(headers, ["reason", "powod", "powód"]);
  const idxPriority = findHeaderIndex_(headers, ["priority", "priorytet"]);
  const idxTimestamp = findHeaderIndex_(headers, ["timestamp", "date", "data", "createdAt"]);
  const idxDecision = findHeaderIndex_(headers, ["therapyDecision", "status"]);

  const dogs = getDogsMap_();
  return values.slice(1).map((row, idx) => {
    const dogId = idxDogId >= 0 ? safeStr(row[idxDogId]) : "";
    const mappedDog = dogs[dogId] || null;
    return {
      id: `report-${idx + 2}`,
      dogId,
      dogName: (idxDogName >= 0 ? safeStr(row[idxDogName]) : "") || mappedDog?.name || "",
      box: mappedDog?.kennel || "",
      pavilion: mappedDog?.pavilion || "",
      reason: idxReason >= 0 ? safeStr(row[idxReason]) : "",
      priority: idxPriority >= 0 ? safeStr(row[idxPriority]) : "",
      timestamp: idxTimestamp >= 0 ? safeStr(row[idxTimestamp]) : "",
      decision: idxDecision >= 0 ? safeStr(row[idxDecision]) || "NEW" : "NEW",
    };
  }).filter((item) => item.dogId || item.dogName);
}

function getTherapyDogs_() {
  const dogs = getDogsMap_();
  return readTherapyRows_().map((row) => ({
    ...row,
    name: dogs[safeStr(row.DogID)]?.name || "",
    box: dogs[safeStr(row.DogID)]?.kennel || "",
  }));
}
